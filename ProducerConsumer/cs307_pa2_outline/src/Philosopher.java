...
public class Philosopher extends Thread {
	...
	public Philosopher(int i, Semaphore [] s, int [] mystate, int philnumber, Semaphore mu, Semaphore[] bar)
	{
		...
		barriers = bar;
		LEFT = (i + 1)%philnumber;
		RIGHT = (i + philnumber -1 )%philnumber;
	}

	public void wait_others(int id){
		//Barrier implementation
		try {

			for(int j = 0; j < 4; j++)
			{
					barriers[id].release(); //Up its own semaphore n-1 times
			}


			for(int j = 0; j < 5; j++)
			{
				if(id != j) //Down all semaphores except its own
				{
					barriers[j].acquire();
				}
			}
	...
	public int test(int id){
			...
			if (state[id] == HUNGRY && state[LEFT] != EATING && state[RIGHT] != EATING) {
				...
				System.out.println("Philosopher " + id + " GRABS  forks.");
				state[id] = EATING;
				table.Eating_GUI(id);
				sem[id].release();
				return EATING;
	...
	public void do_something()
	{
		...
		try
		{
			System.out.println("Thread " + id + " is doing something in " + waitTime + " msecs");
			Thread.sleep(waitTime);
		}
		catch(InterruptedException e)
		{
			System.err.println("interrupted out of doing something");
		}
	}
	...
	public void eat() {
		try {
			...
			System.out.println("Philosopher "+ id + " EATS for " + sleepTime);
	...
	public static void main(String args[]) {
		...
		for (int i=0; i < N; i++)
		{
			...
			oldies[i].start();
			System.out.println("Philosopher #" + i + " entered room");

			try {
				System.out.println("Philosopher #" + i + " walking to table ");
				sleep(1000);
	...
	public void run() {
		...
		while (true) {
			try{
				...
				System.out.println("Hi! I'm philosopher " + id);

				try {
					int sleepTime = ThreadLocalRandom.current().nextInt(0, 10000);
					System.out.println("Philosopher " +id+ " THINKS for " + sleepTime);
					Thread.sleep(sleepTime);
